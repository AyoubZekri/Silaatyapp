  ...
  Future<Map<String, dynamic>> allinvoise() async {
    ...
    final invoices = await dbClient.rawQuery('''
      SELECT 
        ...
        t.phone_number
      FROM invoies i
      INNER JOIN transactions t ON t.uuid = i.Transaction_uuid
      LEFT JOIN sales s ON s.invoie_uuid = i.uuid
      WHERE i.user_id = ?
        AND t.transactions = 2 AND i.is_delete = 0  -- ✅ الفواتير لي تخص زبائن فقط 
      GROUP BY i.uuid
...
''', [id]);
  ...
  Future<Map<String, dynamic>> getMyInvoicesByTransaction(
    ...
    final invoices = await db.readData(
      "SELECT * FROM invoies WHERE user_id = ? AND Transaction_uuid = ? AND is_delete = 0",
      [id, transactionuuId],
    );

    // 2. مجموع الدفع
    final sumPaymentResult = await db.readData(
      "SELECT SUM(Payment_price) as total FROM invoies WHERE user_id = ? AND Transaction_uuid = ? AND is_delete = 0",
      [id, transactionuuId],
    );
    ...
    final transactionResult = await db.readData(
      "SELECT * FROM transactions WHERE user_id = ? AND uuid = ? LIMIT 1 AND is_delete = 0",
      [id, transactionuuId],
    );
    ...
    FROM sales
    WHERE invoie_uuid = ? AND is_delete = 0
  """, [invoiceuuId]);
  ...
  Future<Map<String, dynamic>> switchStatus(Map<String, Object?> data) async {
    try {
      ...
      final existing = await db.readData(
          "SELECT * FROM transactions WHERE uuid = '$uuid' AND is_delete = 0");
